...
```bash
export AGENTCOMMS_API_KEY="ac_live_your_key_here"
...
| [`invoicing-agent/`](invoicing-agent/) | A working Python agent that polls an email inbox, uses AI to categorize and extract invoice data, stores it in SQLite, and replies with a confirmation. Demonstrates the unified inbox + AI features together. |
...
The Python examples (`invoicing-agent/` and `slack-standup-bot/`) use only the standard library plus the `agentcomms` package. Install with:
...
```bash
pytest examples/invoicing-agent/ examples/slack-standup-bot/ -v
```
