...
def make_message(message_id: str = "msg-001", direction: str = "inbound",
    ...
    return {
        ...
        "direction": direction,
        "from": {"name": "Vendor Co", "address": "billing@vendor.com"},
...
def make_agent(agent_id: str = "agt_invoice") -> dict:
    return {
        "agent_id": agent_id,
        "name": "InvoiceBot",
...
def test_invoice_categorize_and_extract():
    ...
    invoice_agent = make_agent("agt_invoice")
    conn = in_memory_db()
    ...
    with patch.object(agent, "_post") as mock_post, \
        ...
        def post_side_effect(path, body):
            if "/ai/categorize" in path:
                return {"message_id": "msg-001", "label": "invoice"}
            if "/ai/extract" in path:
                return {"message_id": "msg-001", "data": {
        ...
        agent.process_message(msg, invoice_agent, conn)
    ...
    reply_body = reply_calls[0].args[1]["body"]
...
def test_non_invoice_skipped():
    ...
    with patch.object(agent, "_post") as mock_post:
        ...
        def post_side_effect(path, body):
            if "/ai/categorize" in path:
                return {"message_id": "msg-002", "label": "other"}
...
def test_large_invoice_sends_alert():
    ...
    with patch.object(agent, "_post") as mock_post, \
        ...
        def post_side_effect(path, body):
            if "/ai/categorize" in path:
                return {"message_id": "msg-003", "label": "invoice"}
            if "/ai/extract" in path:
                return {"message_id": "msg-003", "data": {
            ...
            if "/reply" in path:
                return {"id": "reply-003"}
            # Alert email — POST to /agents/{agent_id}/messages with to=[{ALERT_EMAIL}]
...
def test_outbound_messages_ignored():
    """Outbound messages in the message listing should not be processed."""
