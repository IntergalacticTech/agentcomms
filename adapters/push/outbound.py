...
# adapters/push/outbound.py
"""
Outbound SQS-triggered Lambda for push notifications.
...
queue, loads the Channel, calls PushAdapter.send(), and updates the stored
UnifiedMessage with the SNS MessageId + status.

The outbound_message.to field must be the prefixed endpoint ARN:
  "push:apns:arn:aws:sns:..."  or  "push:fcm:arn:aws:sns:..."
"""
...
def handler(event: dict, context) -> dict:
    ...
    for record in event.get("Records", []):
        ...
        if stored:
            stored.status = (
                MessageStatus.SENT if result.status == "sent" else MessageStatus.FAILED
            )
